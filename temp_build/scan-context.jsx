...
function sr(seed, offset, min, max) {
...
const SKIN_TYPES = [
    {
        type: "Oily",
        emoji: "✨",
        description: "Excess sebum detected across your T-zone and cheeks. Your pores appear visibly enlarged and there is a noticeable shine. This skin type is prone to blackheads, whiteheads and acne breakouts. A balanced, oil-free routine is key.",
    },
    {
        type: "Dry",
        emoji: "💧",
        description: "Low moisture levels detected. Your skin shows signs of tightness, flakiness and reduced elasticity. The skin barrier appears compromised and requires intensive hydration and nourishing lipid-rich products to restore balance.",
    },
    {
        type: "Combination",
        emoji: "⚖️",
        description: "Oily T-zone (forehead, nose and chin) with drier or normal cheeks detected. Oil distribution is uneven across your face. Your routine needs to address both concerns — lightweight formulas on oily zones, richer textures on dry areas.",
    },
    {
        type: "Normal",
        emoji: "🌿",
        description: "Skin appears balanced with no excessive oiliness or dryness detected. Texture is even and complexion looks healthy. Maintain this balance with a gentle, consistent routine focused on protection and light hydration.",
    },
    {
        type: "Sensitive",
        emoji: "🌸",
        description: "Mild redness and potential irritation detected. Your skin appears reactive and may flush or sting easily in response to products, weather or stress. Fragrance-free, barrier-supporting formulas are strongly recommended.",
    },
];
...
// ─── ROUTINE BUILDERS ─────────────────────────────────────────────────────────
function buildIngredients(skinType, topConcerns) {
    const toUse = [];
    const toAvoid = [];
    // Base additions
    ...
    if (topConcerns.includes("Acne & Pimples") || topConcerns.includes("Blackheads")) {
        if (!toUse.find(i => i.name === "Salicylic Acid"))
            toUse.push({ name: "Salicylic Acid", benefit: "Helps unclog pores and reduce acne." });
...
function recommendProducts(skinType, concerns, ingredientsToAvoidNames) {
    ...
    let candidates = PRODUCT_CATALOG.filter(product => {
        ...
        if (!product.suitableSkinTypes.includes(skinType))
            return false;
        // Check avoids
        const hasUnsuitable = product.unsuitableIngredients.some(un => ingredientsToAvoidNames.some(avoid => avoid.toLowerCase().includes(un.toLowerCase())));
        if (hasUnsuitable)
            return false;
        return true;
    });
...
function buildMorningRoutine(recommendedProducts) {
    const steps = [];
    const cleanser = recommendedProducts.find(p => p.category === "Cleanser" && p.routineTime !== "Night");
    if (cleanser)
        steps.push({ step: steps.length + 1, product: `${cleanser.brand} ${cleanser.name}`, instruction: cleanser.usageInstructions, timing: "60 sec" });
    else
        steps.push({ step: steps.length + 1, product: "Gentle Cleanser", instruction: "Wash face gently to remove overnight oils.", timing: "60 sec" });
    const toner = recommendedProducts.find(p => p.category === "Toner" && p.routineTime !== "Night");
    if (toner)
        steps.push({ step: steps.length + 1, product: `${toner.brand} ${toner.name}`, instruction: toner.usageInstructions, timing: "Immediate" });
    const serum = recommendedProducts.find(p => p.category === "Serum" && p.routineTime !== "Night");
    if (serum)
        steps.push({ step: steps.length + 1, product: `${serum.brand} ${serum.name}`, instruction: serum.usageInstructions, timing: "2 min" });
    else
        steps.push({ step: steps.length + 1, product: "Antioxidant Serum (Optional)", instruction: "Apply a Vitamin C serum to protect against environmental damage.", timing: "2 min" });
    const moisturizer = recommendedProducts.find(p => p.category === "Moisturizer" && p.routineTime !== "Night");
    if (moisturizer)
        steps.push({ step: steps.length + 1, product: `${moisturizer.brand} ${moisturizer.name}`, instruction: moisturizer.usageInstructions, timing: "2 min" });
    else
        steps.push({ step: steps.length + 1, product: "Daily Moisturizer", instruction: "Apply an even layer to lock in hydration.", timing: "2 min" });
    const sunscreen = recommendedProducts.find(p => p.category === "Sunscreen");
    if (sunscreen)
        steps.push({ step: steps.length + 1, product: `${sunscreen.brand} ${sunscreen.name}`, instruction: sunscreen.usageInstructions, timing: "Final step" });
    else
        steps.push({ step: steps.length + 1, product: "Broad Spectrum SPF 30+", instruction: "Apply liberally as the final step. Reapply every 2 hours.", timing: "Final step" });
    return steps;
}
function buildNightRoutine(recommendedProducts) {
    const steps = [];
    const cleanser = recommendedProducts.find(p => p.category === "Cleanser");
    if (cleanser)
        steps.push({ step: steps.length + 1, product: `Double Cleanse (Use ${cleanser.brand} ${cleanser.name} for 2nd wash)`, instruction: "Use an oil-based cleanser first to break down SPF/makeup, then follow with your water-based cleanser.", timing: "2 min total" });
    else
        steps.push({ step: steps.length + 1, product: "Double Cleanse", instruction: "Remove makeup/SPF with a balm, followed by a gentle cleanser.", timing: "2 min total" });
    ...
    if (treatment)
        steps.push({ step: steps.length + 1, product: `${treatment.brand} ${treatment.name}`, instruction: treatment.usageInstructions, timing: "Wait 2-3 mins" });
    else
        steps.push({ step: steps.length + 1, product: "Treatment Serum", instruction: "Apply targeted treatments (e.g., Retinol, AHA/BHA) here.", timing: "3 mins" });
    const moisturizer = recommendedProducts.find(p => p.category === "Moisturizer");
    if (moisturizer)
        steps.push({ step: steps.length + 1, product: `${moisturizer.brand} ${moisturizer.name}`, instruction: moisturizer.usageInstructions, timing: "Final step" });
    else
        steps.push({ step: steps.length + 1, product: "Night Cream", instruction: "Apply a nourishing layer to seal in hydration and actives.", timing: "Final step" });
    return steps;
}
function buildWeeklyPlan(skinType) {
...
function buildLifestyleTips(skinType, healthScore) {
    return [
...
function checkImageQuality(url) {
    return url.length > 5000 ? "good" : "poor";
}
export async function analyzeImageStandalone(imageDataUrl, method) {
    ...
    const concerns = CONCERN_DEFS.map((def, i) => {
    ...
    const spfNote = skinType === "Sensitive"
    ...
    return {
        ...
        lifestyleTips,
        weeklyPlan,
        disclaimer: "⚕️ This analysis is AI-generated for informational purposes only. It is not a substitute for professional medical or dermatological advice, diagnosis, or treatment. Always consult a qualified dermatologist for skin concerns.",
    };
}
// ─── PROVIDER ─────────────────────────────────────────────────────────────────
export const ScanProvider = ({ children }) => {
    ...
    const analyzeImage = async (imageDataUrl, method) => {
        const report = await analyzeImageStandalone(imageDataUrl, method);
        setCurrentScan(report);
        return report;
    };
    const clearCurrentScan = () => setCurrentScan(null);
    return (<ScanContext.Provider value={{ scanHistory, currentScan, analyzeImage, saveScan, clearCurrentScan }}>
      {children}
